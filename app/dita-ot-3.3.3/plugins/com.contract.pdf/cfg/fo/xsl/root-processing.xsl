...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!--Year Month Date-->
    <xsl:variable name="bc.currentDate" select="format-date(current-date(), '[M01]/[D01]/[Y0001]')" />
    ...
    <xsl:variable name="bc.productVersion">
    	<xsl:variable name="mapVersion" select="(/*/opentopic:map//*[contains(@class, ' topic/vrm ')]/@version)[1]"/>
    	<xsl:choose>
    		<xsl:when test="$mapVersion">
...
<!--added 2019/05/16-->
       <!--Series
       <xsl:variable name="bc.seriesText">
        	<xsl:variable name="seriesText" select="(/*/opentopic:map//*[contains(@class, ' topic/series ')])[1]" as="element()?"/>
        	<xsl:choose>
              	<xsl:when test="exists($seriesText)">
                		<xsl:apply-templates select="$seriesText"/>
            		</xsl:when>
            		<xsl:otherwise>
                		<xsl:call-template name="getVariable">
                    			<xsl:with-param name="id" select="'Series Text'"/>
                		</xsl:call-template>
            		</xsl:otherwise>
        	</xsl:choose>
    	</xsl:variable>-->
    ...
    <!-- added 2019/04/24 -->
    <xsl:variable name="bc.pubDate" select="(/*/opentopic:map//*[contains(@class, ' topic/revised ')]/@golive)[1]"/>
    ...
    <xsl:variable name="bc.bookTitle">
        <xsl:value-of>
            <xsl:apply-templates select="$map/topicmeta/navtitle" mode="dita-ot:text-only"/>
        </xsl:value-of>
    </xsl:variable>

    <!--Book Short Description-->
        <xsl:variable name="bc.shortDesc">
        <xsl:value-of>
            <xsl:apply-templates select="$map/topicmeta/shortdesc" mode="dita-ot:text-only"/>
        </xsl:value-of>
    </xsl:variable>
    ...
    <xsl:variable name="bc.copyYear">
      <xsl:choose>
        <xsl:when test="$map//*[contains(@class, ' topic/data bookmap/copyright ')]//*[contains(@class, ' topic/ph bookmap/year ')]">
    ...
    <xsl:variable name="bc.itemNumber" select="//*[contains(@class, ' bookmap/bookid ')]//*[contains(@class, ' bookmap/booknumber ')]"/>
